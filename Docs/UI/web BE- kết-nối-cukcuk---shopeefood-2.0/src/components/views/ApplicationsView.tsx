...
export const ApplicationsView: React.FC<ApplicationsViewProps> = ({
  ...
  const [quickLinkTab, setQuickLinkTab] = useState<"dish" | "stpv">("dish");
  ...
  // Backup of original ShopeeFood Wizard mock data to restore when switched to has_store
  const BACKUP_WIZARD_FOODS = [
    {
      id: 1,
      name: "Bánh mì thịt nướng",
      category: "Món ăn nhẹ",
  ...
  const BACKUP_WIZARD_MENU_GROUPS = [
    { id: "wmg1", name: "Món chính", description: "Các món chính của cửa hàng", linkedGroupId: "ccmg1" },
    { id: "wmg2", name: "Món ăn nhẹ", description: "Các món ăn vặt nhẹ", linkedGroupId: "" },
    { id: "wmg3", name: "Đồ uống lạnh", description: "Đồ uống giải khát", linkedGroupId: "ccmg3" },
    { id: "wmg4", name: "Món chè", description: "Các loại chè tráng miệng", linkedGroupId: "" },
  ...
  const BACKUP_WIZARD_STPV_GROUPS = [
    { id: "wsog1", name: "Topping", description: "Các loại topping thêm kèm", linkedGroupId: "ccsg1" },
    { id: "wsog2", name: "Tùy chọn", description: "Tùy chọn độ ngọt, đá", linkedGroupId: "" },
    { id: "wsog3", name: "Size", description: "Kích thước cốc", linkedGroupId: "ccsg3" },
  ];

  const BACKUP_WIZARD_STPV = [
    {
      id: "stpv1",
      name: "Oreo",
      group: "Topping",
  ...
  // Dynamic clear/restore wizard data based on connection type (No store means fresh store with empty SPF menu)
  useEffect(() => {
    if (connectionType === "no_store" || isNoStoreFlow) {
      setWizardFoods([]);
      setWizardMenuGroups([]);
      setWizardStpv([]);
      setWizardStpvGroups([]);
    } else {
      setWizardFoods(BACKUP_WIZARD_FOODS);
      setWizardMenuGroups(BACKUP_WIZARD_MENU_GROUPS);
      setWizardStpv(BACKUP_WIZARD_STPV);
      setWizardStpvGroups(BACKUP_WIZARD_STPV_GROUPS);
    }
  }, [connectionType, isNoStoreFlow]);

  // Simulate scanning of ShopeeFood QR Code
  useEffect(() => {
    let timer1: NodeJS.Timeout;
    let timer2: NodeJS.Timeout;
    if (isQrModalOpen && connectionType === "has_store") {
  ...
  }, [isQrModalOpen, connectionType]);

  // Simulate menu synchronization for Step 2 (instantly)
  useEffect(() => {
                ...
                {shopeeWizardStep === 1 &&
                  (qrScanStatus === "success" ? (
                    ...
                    <div className="flex flex-col items-center justify-center text-center animate-success-pop">
                      ...
                      <div className="flex items-center justify-center h-[230px] select-none">
                        <svg
                          width="310"
                          height="230"
                          viewBox="0 0 310 230"
                            ...
                            <>
                              ...
                              <div className="text-center text-[#717680] text-[13px] leading-relaxed max-w-[620px] px-4 font-sans space-y-2">
                                ...
                                <div className="text-[12px] text-amber-600 font-medium">
                                  ...
                                  {qrScanStatus === "scanning" && "● Đang tiến hành quét và xác minh thông tin..."}
                                </div>
                              </div>
                            </>
                        </div>
                  ...
                  (step2SyncProgress < 100 ? (
                    ...
                    <div className="flex flex-col items-center justify-center w-full max-w-2xl text-center space-y-6 animate-fade-in p-6 mx-auto">
                      ...
                      <div className="w-full max-w-[520px] flex flex-col items-center animate-fade-in">
                        ...
                        {/* Title */}
                        <h4 className="text-[#101828] font-bold text-[20px] font-sans mb-2 text-center tracking-tight flex items-center justify-center gap-2">
                          <RefreshCw className="w-5 h-5 text-[#245FDF] animate-spin" />
                    ...
                    <div className="flex-1 flex flex-col overflow-hidden w-full h-full text-left bg-white">
                      ...
                      <div className="px-6 py-4 flex items-center justify-between select-none">
                        <div>
                          <h3 className="text-[#101828] font-bold text-[16px] font-sans flex items-center gap-1.5">
                        ...
                        <div className="flex gap-2">
                          <button
                            onClick={() => {
                              onNotification("Bắt đầu đồng bộ thực đơn từ ShopeeFood...", "info");
                            ...
                            className="bg-white hover:bg-gray-50 text-[#344054] border border-[#D0D5DD] font-medium text-[13px] px-4 rounded-[8px] flex items-center justify-center gap-1.5 transition-all h-[32px] cursor-pointer shadow-sm font-sans flex-shrink-0"
                          >
                            <RefreshCw className="w-4 h-4 text-[#475467]" />
                            Đồng bộ
                          </button>
                          <button
                            onClick={() => {
                                      ...
                                      {isNoStoreFlow && wizardFoods.length === 0 ? (
                                        <div className="flex flex-col items-center justify-center space-y-3 py-4 max-w-lg mx-auto">
                                          ...
                                          <div className="space-y-1">
                                            <p className="font-bold text-[#101828] text-[14px]">Để trống dữ liệu món</p>
                                            <p className="text-[12px] text-[#717680] leading-relaxed">
                                              Muốn thêm thì chọn vào <strong className="text-[#245FDF] font-semibold">Chọn món</strong>
                                            </p>
                                          </div>
                                        </div>
                                      ) : (
                                    ...
                                    >
                                      {isNoStoreFlow && wizardStpv.length === 0 ? (
                                        <div className="flex flex-col items-center justify-center space-y-3 py-4 max-w-lg mx-auto">
                                          ...
                                          <div className="space-y-1">
                                            <p className="font-bold text-[#101828] text-[14px]">Chưa có sở thích phục vụ</p>
                                            <p className="text-[12px] text-[#717680] leading-relaxed">
                                              Sở thích phục vụ sẽ tự động được thêm sau khi bạn bấm <strong className="text-[#245FDF] font-semibold">"Chọn món"</strong> ở góc trên bên phải để chọn món có sở thích phục vụ đi kèm từ MISA CukCuk sang ShopeeFood.
                                            </p>
                                          </div>
                                        </div>
                                      ) : (
                                        <span>Không tìm thấy sở thích phục vụ nào phù hợp với bộ lọc.</span>
                                      )}
                                    </td>
                                  </tr>
                                )}
                    ...
                    >
                      ...
                      <div className="h-[56px] px-4 py-3 flex items-center border-b border-[#E9EAEB] flex-shrink-0 gap-2 overflow-x-auto">
                        ...
                        <button
                          ...
                          className="bg-white hover:bg-[#EDFCF4] text-[#245FDF] border border-[#245FDF]/30 font-medium text-[13px] px-3.5 h-[32px] rounded-[8px] flex items-center justify-center cursor-pointer transition-colors flex-shrink-0"
                          style={{ minWidth: "84px" }}
                        >
                          Liên kết món nhanh
                        </button>
                                    ...
                                    paginatedItems.map((item) => {
                                      ...
                                      return (
                                        ...
                                        >
                                          ...
                                          <td className="px-4 text-left border-r border-[#E9EAEB]">
                                            <span
                                              className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-[4px] text-[12px] font-medium border ${
                                                status === "Sử dụng"
                                                  ? "bg-[#EDFCF4] text-[#12B76A] border-[#12B76A]"
                                                  : "bg-[#F3F4F6] text-[#717680] border-[#D5D7DA]"
                                              }`}
                                            >
                                              {status}
                                            </span>
                                          ...
                                          {/* Trạng thái liên kết */}
                                          <td className="px-4 text-left border-r border-[#E9EAEB]" onClick={(e) => e.stopPropagation()}>
                                            <span className="inline-flex items-center gap-1.5 text-[13px] font-semibold font-sans">
                                              <span
                                                className={`w-2 h-2 rounded-full flex-shrink-0 ${
                                                  isLinked
                                                ...
                                                {isLinked ? (
                                                  <span>
                                                    ...
                                                    <span className="font-normal text-gray-500">
                                                      ({linkedCukCukGroup?.name || item.name})
                                                    </span>
                                                  </span>
                                                ) : (
                                                  "Chưa liên kết"
  ...
  function renderShopeeEditModal() {
    ...
    function renderQuickLinkModal() {
      ...
      const copyDishToCukCuk = (item: any) => {
        const newId = `cc_auto_quick_${item.id}`;
        const newDishName = item.name;
        const codePrefix = item.category.slice(0, 2).toUpperCase() || "MA";
        const randomNum = Math.floor(10 + Math.random() * 90);
        const newCode = `${codePrefix}${randomNum}`;

        setWizardCukCukDishes((prev) => {
          if (prev.some((d) => d.id === newId)) return prev;
          return [...prev, { id: newId, name: newDishName, code: newCode }];
        });
      ...
      const matchedDishesCount = quickLinkEditingItems.filter((item) =>
        wizardCukCukDishes.some((cc) => cc.name.toLowerCase() === item.name.toLowerCase())
      ).length;
      const totalDishesCount = quickLinkEditingItems.length;
      const unlinkedMatchedDishesCount = quickLinkEditingItems.filter((item) => {
        const hasMatch = wizardCukCukDishes.some((cc) => cc.name.toLowerCase() === item.name.toLowerCase());
        return hasMatch && !item.linkedDishId;
      }).length;

      const matchedStpvCount = quickLinkEditingStpv.filter((item) =>
        wizardCukCukGroups.some((cc) => cc.name.toLowerCase() === item.name.toLowerCase())
      ).length;
      const totalStpvCount = quickLinkEditingStpv.length;
      const unlinkedMatchedStpvCount = quickLinkEditingStpv.filter((item) => {
        const hasMatch = wizardCukCukGroups.some((cc) => cc.name.toLowerCase() === item.name.toLowerCase());
        return hasMatch && !item.linkedGroupId;
      }).length;

      const unlinkedDishesCount = quickLinkEditingItems.filter(item => !item.linkedDishId).length;
      const unlinkedStpvsCount = quickLinkEditingStpv.filter(item => !item.linkedGroupId).length;

      // Select active dataset & filters based on active tab
      const filteredItems = quickLinkTab === "dish" 
        ? quickLinkEditingItems.filter((item) => {
            const spfCode = `SPF-${String(item.id).padStart(3, "0")}`;
            
            if (quickLinkSearchText) {
              const q = quickLinkSearchText.toLowerCase();
              const matchesName = item.name.toLowerCase().includes(q);
              const matchesCode = spfCode.toLowerCase().includes(q);
              if (!matchesName && !matchesCode) return false;
            }

            if (quickLinkCukCukSearchText) {
              const q = quickLinkCukCukSearchText.toLowerCase();
              const matchedCC = wizardCukCukDishes.find((cc) => cc.id === item.linkedDishId);
              if (!matchedCC) return false;
              const matchesCcName = matchedCC.name.toLowerCase().includes(q);
              const matchesCcCode = (matchedCC.code || "").toLowerCase().includes(q);
              if (!matchesCcName && !matchesCcCode) return false;
            }

            if (quickLinkCategoryFilter) {
              const q = quickLinkCategoryFilter.toLowerCase();
              if (!(item.category || "").toLowerCase().includes(q)) return false;
            }

            if (quickLinkUnitFilter) {
              const q = quickLinkUnitFilter.toLowerCase();
              if (!(item.unit || "").toLowerCase().includes(q)) return false;
            }

            if (quickLinkStatusFilter === "linked") {
              if (!item.linkedDishId) return false;
            } else if (quickLinkStatusFilter === "unlinked") {
              if (item.linkedDishId) return false;
            }

            return true;
          })
        : quickLinkEditingStpv.filter((item) => {
            if (quickLinkSearchText) {
              const q = quickLinkSearchText.toLowerCase();
              const matchesName = item.name.toLowerCase().includes(q);
            ...
            if (quickLinkCukCukSearchText) {
              const q = quickLinkCukCukSearchText.toLowerCase();
              const matchedCC = wizardCukCukGroups.find((cc) => cc.id === item.linkedGroupId);
              if (!matchedCC) return false;
              const matchesCcName = matchedCC.name.toLowerCase().includes(q);
              const matchesCcCode = (matchedCC.code || "").toLowerCase().includes(q);
              if (!matchesCcName && !matchesCcCode) return false;
            ...
            if (quickLinkStpvGroupFilter) {
              const q = quickLinkStpvGroupFilter.toLowerCase();
              if (!(item.group || "").toLowerCase().includes(q)) return false;
            }
      ...
      const currentUnlinkedMatchedCount = quickLinkTab === "dish" ? unlinkedMatchedDishesCount : unlinkedMatchedStpvCount;
      const currentMatchedCount = quickLinkTab === "dish" ? matchedDishesCount : matchedStpvCount;
      const currentTotalCount = quickLinkTab === "dish" ? totalDishesCount : totalStpvCount;
      const currentItemTypeLabel = quickLinkTab === "dish" ? "món" : "STPV";
      ...
      return (
        <div className="fixed inset-0 bg-[#F0F2F4] z-[10030] flex flex-col font-sans select-none overflow-hidden text-left animate-fade-in">
          ...
          {/* Subtab Navigation (Món & Sở thích phục vụ) */}
          <div className="bg-white border-b border-[#E9EAEB] px-6 h-12 flex items-center gap-6 flex-shrink-0">
            <button
              type="button"
              onClick={() => {
                setQuickLinkTab("dish");
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <span className="text-[#717680] text-[13px] font-sans ml-1">
                      Có <strong className="text-[#245FDF] font-semibold">{currentUnlinkedMatchedCount}</strong> {currentItemTypeLabel} chưa liên kết trên ShopeeFood trùng tên với {quickLinkTab === "dish" ? "món" : "STPV"} tại MISA CukCuk. Bạn có thể thực hiện Liên kết nhanh các {currentItemTypeLabel} này.
                    </span>
                  </div>
                </div>
                <button
                  type="button"
                  onClick={quickLinkTab === "dish" ? autoLinkDishes : autoLinkStpvs}
                    ...
                    <>
                      ...
                      <tbody className="divide-y divide-[#E9EAEB]">
                        {paginatedItems.length > 0 ? (
                          ...
                          <tr>
                            <td colSpan={4} className="text-center py-10 text-[#717680] text-[13px]">
